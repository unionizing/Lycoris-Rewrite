...
return function(self, timing)
	...
	local tracker = ProjectileTracker.new(function(candidate)
		return candidate.Name:match("PumpkinProjectile")
	end)
	...
	if self:distance(self.entity) <= 20 then
		...
		action._type = "Parry"
		action._when = 0
		action.name = "Pumpkin Throw Close Timing"
		action.ihbc = true
	...
	action._when = 0
	action._type = "Parry"
	action.name = "Pumpkin Throw Part"

	local pt = PartTiming.new()
	pt.uhc = true
	pt.duih = true
	pt.fhb = false
	pt.name = "PumpinProjectile"
	pt.hitbox = Vector3.new(30, 30, 40)
